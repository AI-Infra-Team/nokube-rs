        ...
        for volume in &self.volumes {
            ...
            if !host_path.exists() {
                return Err(NokubeError::FileOperation {
                    ...
                    reason: format!(
                        "Volume mount source path '{}' does not exist",
                        volume.host_path
                    ),
                }
                .into());
            ...
            if host_path.is_file() {
                ...
                return Err(NokubeError::FileOperation {
                    file_path: volume.host_path.clone(),
                    reason: format!(
                        "Volume mount source path '{}' is neither a file nor directory",
                        volume.host_path
                    ),
                }
                .into());
...
impl DockerRunner {
    ...
    pub fn find_container_runtime() -> Result<ContainerRuntime> {
        ...
        let candidates: Vec<(&str, &[&str])> = vec![
            ...
            (
                "nerdctl",
                &["/usr/bin/nerdctl", "/usr/local/bin/nerdctl", "/bin/nerdctl"],
            ),
        ];
    ...
    pub fn run(config: &DockerRunConfig) -> Result<String> {
        ...
        info!(
            "Using container runtime '{}' at path: {}",
            runtime.name, runtime.path
        );
        ...
        let output = command
            .output()
            .map_err(|e| NokubeError::ServiceDeploymentFailed {
        ...
        info!(
            ...
            config.container_name, container_id
        );
    ...
    pub fn remove_container(container_name: &str) -> Result<()> {
        ...
        if !output.status.success() {
            ...
            info!(
                "Note: Failed to remove container '{}': {}",
                container_name,
                error_msg.trim()
            );
        }
...
mod tests {
    ...
    fn test_docker_config_validation() {
        ...
        // 测试不存在的挂载路径
        let config = DockerRunConfig::new("test".to_string(), "nginx".to_string()).add_volume(
