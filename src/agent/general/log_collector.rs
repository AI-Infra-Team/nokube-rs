...
use opentelemetry_otlp::{LogExporter, ExportConfig};
...
fn get_or_leak_scope_name(s: &str) -> &'static str {
    ...
    if let Some(&v) = map.get(s) { return v; }
...
impl LogCollector {
    pub fn new(config: LogCollectorConfig) -> Result<Self> {
        ...
        // 使用环境变量为 OTLP 请求添加 Greptime 所需请求头
...
X-Greptime-Log-Extract-Keys=cluster_name,node_name,source,source_id,container,pod,root_actor,container_path";
        std::env::set_var("OTEL_EXPORTER_OTLP_HEADERS", headers_str);
        std::env::set_var("OTEL_EXPORTER_OTLP_LOGS_HEADERS", headers_str);

        // 配置 OTLP 超时（毫秒）: 优先使用传入配置
        let timeout_ms = (config.flush_timeout_secs.saturating_mul(1000)).to_string();
        std::env::set_var("OTEL_EXPORTER_OTLP_TIMEOUT", &timeout_ms);
        std::env::set_var("OTEL_EXPORTER_OTLP_LOGS_TIMEOUT", &timeout_ms);
        if let (Ok(u), Ok(p)) = (
            std::env::var("NOKUBE_GREPTIME_HTTP_USER"),
            std::env::var("NOKUBE_GREPTIME_HTTP_PASSWORD"),
        ) {
            if !u.is_empty() {
                let token = base64::engine::general_purpose::STANDARD.encode(format!("{}:{}", u, p));
                let append = format!(",Authorization=Basic {}", token);
                let mut v = std::env::var("OTEL_EXPORTER_OTLP_HEADERS").unwrap_or_else(|_| headers_str.to_string());
                v.push_str(&append);
                std::env::set_var("OTEL_EXPORTER_OTLP_HEADERS", &v);
                std::env::set_var("OTEL_EXPORTER_OTLP_LOGS_HEADERS", &v);
            }
        }
    ...
    pub async fn start(&mut self) -> Result<()> {
        info!("Starting log collector for cluster: {}", self.config.cluster_name);
    ...
    async fn flush_logs_otlp(logs: &[LogEntry], logger_provider: &LoggerProvider, timeout_secs: u64) {
        if logs.is_empty() {
            return;
        }
        
        info!("Flushing3 {} log entries to GreptimeDB using OTLP", logs.len());

        // 外层超时保护：防止卡住导致既无成功也无失败
        let timeout_duration = Duration::from_secs(timeout_secs);
        match tokio::time::timeout(timeout_duration, Self::try_flush_logs(logs, logger_provider)).await {
            Ok(Ok(())) => {
    ...
    async fn try_flush_logs(logs: &[LogEntry], logger_provider: &LoggerProvider) -> Result<()> {
        ...
        for log_entry in logs {
            ...
            // 设置基本属性
            log_record.timestamp = Some(std::time::SystemTime::UNIX_EPOCH + std::time::Duration::from_nanos(log_entry.timestamp.timestamp_nanos_opt().unwrap_or(0) as u64));
            ...
            if log_entry.source == "actor" {
                ...
                let (pod_name, root_actor, _owner_type) = crate::agent::general::exporter::Exporter::derive_actor_core(
                    &log_entry.source_id,
                    &log_entry.node_name,
        ...
        for (index, result) in results.iter().enumerate() {
            match result {
                ...
                Err(e) => {
                    error_count += 1;
                    error!("Failed to flush log batch {} to GreptimeDB: {:?}", index + 1, e);
                    // 尝试提取更多错误细节
        ...
        if result_count > 0 {
            info!("Flushed {} log entries: {} successful, {} failed", 
                logs.len(), success_count, error_count);
        } else {
            // 未返回任何结果——对此进行显式告警，便于现场排查（常见于头部/鉴权/endpoint 配置问题）
            let hdr_all = std::env::var("OTEL_EXPORTER_OTLP_HEADERS").unwrap_or_else(|_| "<unset>".to_string());
            let hdr_logs = std::env::var("OTEL_EXPORTER_OTLP_LOGS_HEADERS").unwrap_or_else(|_| "<unset>".to_string());
            warn!(
        ...
        if error_count > 0 {
            if let Some(err) = last_error {
                ...
                return Err(anyhow::anyhow!("Log flush failed with {} errors", error_count));
            }
        }
    ...
    pub async fn collect_docker_logs(&self, container_name: &str) -> Result<()> {
        info!("Starting to collect Docker logs for container: {}", container_name);
                    ...
                    for line in stdout.lines() {
                        if !line.trim().is_empty() {
                            ...
                            let entry = LogEntry {
                                ...
                                fields: {
                                    ...
                                    if is_actor {
                                        ...
                                        let path = format!("{}/{}/{}/{}", cluster_name, root_actor, pod_name, container);
                            ...
                            if let Err(e) = sender.send(entry).await {
                                ...
                                debug!("Sent Docker stdout log entry: container={}, level=INFO", container);
                            }
                        }
                    }
                    ...
                    for line in stderr.lines() {
                        if !line.trim().is_empty() {
                            ...
                            if let Err(e) = sender.send(entry).await {
                                ...
                                debug!("Sent Docker stderr log entry: container={}, level=ERROR", container);
                            }
                        }
                    }
                }
    ...
    pub async fn follow_docker_logs(&self, container_name: &str) -> Result<()> {
        info!("Starting to follow Docker logs for container: {}", container_name);
        ...
        tokio::spawn(async move {
            loop {
                ...
                match cmd.spawn() {
                    ...
                    Err(e) => {
                        error!("Failed to spawn docker logs command for {}: {}", container, e);
                    }
                }
